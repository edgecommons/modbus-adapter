...
def test_defines_metric_and_emits_the_exact_section5_measure_set():
    ...
    counters.increment_write_error(3)
    counters.increment_reconnect(2)
    counters.set_signals_serving(7)
    ...
    assert set(values) == set(HEALTH_MEASURES)                    # the exact §5 eight-measure set
    ...
    assert values["writeErrors"] == 3.0
    assert values["signalsSubscribed"] == 7.0                     # served inventory while connected
    # interval counters (readErrors/reconnects/writeErrors) reset on emit; the connection gauge
    # follows the argument, and signalsSubscribed drops to 0 while disconnected
    h.emit(False)
    second = metrics.emitted[1][1]
    assert second["connectionState"] == 0.0 and second["readErrors"] == 0.0
    assert second["reconnects"] == 0.0 and second["writeErrors"] == 0.0
    assert second["signalsSubscribed"] == 0.0
