"""One Modbus signal: a named, typed view onto a register/bit address."""
from .. import codec
from .deadband_spec import DeadbandSpec


class SignalSpec:
    """A single signal. ``table``+``address`` locate it; ``type`` (+ order/scale/bit) decode its value.

    Unlike OPC UA (which discovers nodes), Modbus signals are declared explicitly — there is no browse.
    """
    ...
    def from_dict(o):
        name = o.get("name")
        if not name:
            raise ValueError(f"signal missing 'name': {o}")
        table = o.get("table")
        if table not in codec.TABLES:
            raise ValueError(f"signal '{name}': table must be one of {sorted(codec.TABLES)}, got {table!r}")
        if "address" not in o:
            raise ValueError(f"signal '{name}': missing 'address'")
        address = int(o["address"])
        if address < 0:
            raise ValueError(f"signal '{name}': address must be >= 0")
        # bit tables are always bool; register tables default to uint16
        if table in codec.BIT_TABLES:
            type_ = o.get("type", "bool")
            if type_ != "bool":
                raise ValueError(f"signal '{name}': {table} signals must be type 'bool'")
        else:
            type_ = o.get("type", "uint16")
            if type_ not in codec.TYPES:
                raise ValueError(f"signal '{name}': unknown type {type_!r}")
        count = o.get("count")
        if type_ == "string" and not count:
            raise ValueError(f"signal '{name}': 'string' type requires 'count'")
        bit = o.get("bit")
        if bit is not None and (table in codec.BIT_TABLES or type_ != "bool"):
            raise ValueError(f"signal '{name}': 'bit' is only valid for a bool on a holding/input register")
        return SignalSpec(
    ...
    def address_dict(self, unit_id) -> dict:
        """The protocol-native ``signal.address`` published in the envelope."""
    ...
    def signal_id(self, unit_id) -> str:
        """Stable canonical id, e.g. ``u1/holding/40/float32`` (the OPC UA ``signal.id`` analog)."""
