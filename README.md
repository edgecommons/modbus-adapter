...
`sb/status`, `sb/signals`, `reconnect`, `repoll`), emits `evt/connection`/`evt/write` events, and
reports the `southbound_health` metric. It runs wherever you deploy it — a Greengrass v2 component, a
standalone process, or a Kubernetes pod.
